package com.example.seckillgoods.controller;

import com.example.seckillgoods.entity.TOrder;
import com.example.seckillgoods.entity.TSeckillOrder;
import com.example.seckillgoods.entity.TUser;
import com.example.seckillgoods.exception.GlobalException;
import com.example.seckillgoods.service.ITGoodsService;
import com.example.seckillgoods.service.ITOrderService;
import com.example.seckillgoods.service.ITSeckillOrderService;
import com.example.seckillgoods.vo.GoodsVo;
import com.example.seckillgoods.vo.RespBean;
import com.example.seckillgoods.vo.RespBeanEnum;
import com.example.seckillgoods.vo.SeckillMessage;
...
//        model.addAttribute("goods", goodsVo);
    ...
    public String doSecKillOrder(Model model, TUser user, Long goodsId, @CookieValue("userTicket") String ticket) {
        try {
            System.out.print("获取ticket：" + ticket);
            model.addAttribute("user", user);
            GoodsVo goodsVo = itGoodsService.findGoodsVobyGoodsId(goodsId);
            if (goodsVo.getStockCount() < 1) {
                model.addAttribute("errmsg", RespBeanEnum.EMPTY_STOCK.getMessage());
    ...
    @RequestMapping(value = "/doSeckill2c", method = RequestMethod.POST)
    public String doSecKill2(Model model, TUser user, Long goodsId, @CookieValue("userTicket") String ticket) {
        try {
            System.out.print("获取ticket：" + ticket);
            model.addAttribute("user", user);
            GoodsVo goodsVo = itGoodsService.findGoodsVobyGoodsId(goodsId);
    ...
    @ResponseBody
    public RespBean createOrder(String userId, String goodsId) {
        TOrder order = orderService.createOrder(userId, goodsId);
